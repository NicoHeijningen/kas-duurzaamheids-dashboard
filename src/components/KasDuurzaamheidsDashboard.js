...
const KasDuurzaamheidsDashboard = () => {
  ...
  const renderSettingsTab = () => (
    <div className="space-y-6 p-6">
      <h3 className="text-xl font-bold mb-6">Energiebron Instellingen</h3>
        ...
        {energyData.map((source, index) => (
          <div key={source.name} className="bg-white p-4 rounded-lg shadow-sm">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor={`price-${index}`}>Prijs per MWh (€)</Label>
                <Input
                  id={`price-${index}`}
                  type="number"
                  value={source.pricePerMWh}
                  onChange={(e) => handleValueChange(index, 'pricePerMWh', e.target.value)}
                  className="mt-1"
                />
              </div>
              <div>
                <Label htmlFor={`co2-${index}`}>CO2 Uitstoot per MWh (ton)</Label>
                <Input
                  id={`co2-${index}`}
                  type="number"
                  value={source.co2PerMWh}
                  onChange={(e) => handleValueChange(index, 'co2PerMWh', e.target.value)}
                  className="mt-1"
                  step="0.01"
                />
              </div>
      ...
      <Card className="max-w-7xl mx-auto shadow-xl">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="dashboard">
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6"></div>
